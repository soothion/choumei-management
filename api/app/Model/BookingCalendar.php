...
class BookingCalendar extends Model {
	...
	{
	    Utils::log("logs",date("Y-m-d H:i:s ").($change_type == self::CHANGE_TYPE_OF_ADD ?"ADD":"DEL") .": items:[".implode(",", $item_ids)."] date:{$booking_date} \n");
	    if(count($item_ids)<1)
	    {
	        return;
	    }	   
	...
	{
	    Utils::log("logs",date("Y-m-d H:i:s ").($change_type == self::CHANGE_TYPE_OF_ADD ?"ADD":"DEL") .": items:[".implode(",", $item_ids)."] date:{$arrive_date} \n");
	    if(count($item_ids)<1)
	    {
	        return;
	    }
